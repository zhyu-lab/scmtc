function callcnas(inputFile,embeddingFile,outputDir,maxCN,K)

%------Input and output------%
% inputFile: file containing raw read counts of all cells
% embeddingFile: file containing low-dimensional embeddings of cells
% outputDir: directory of output files
% maxCN: maximum copy number to consider
% K: number of nearest neighbors to smooth data 
...
disp(['number of nearest neighbors is set to ' num2str(K)]);
...
max_iter = 100;
verbose = 0;
init_paras = [{[]},{[]},{[]},{[]},{[]}]; % initial parameters will be assigned in the main function
...
%--------------load data--------------------
[data_lrc_all,data_chr_all,data_bin_all,bin_size] = load_data(inputFile,embeddingFile,K);
...
data_paras_all = zeros(num_cell,1+sum(depend_table(:,2) == 1));
...
for k = 1:num_cell
    ...
    %------------------ call cnas --------------------
    paras = main(init_paras,depend_table,stepsize_ds,thres_EM,max_iter,verbose);

    o = paras{3}{1};
    sigma = paras{4}{1};
    data_paras_all(k,:) = [o sigma];

    [p_states,aCN,segments] = process_results(depend_table);
...
fprintf(o_fid,'o,sigma\n');
S = size(data_paras_all,2)-1;
for k = 1:num_cell
    fprintf(o_fid,'%.6f,[',data_paras_all(k,1));
    for s = 1:S-1
        fprintf(o_fid,'%.6f,',data_paras_all(k,1+s));
    end
    fprintf(o_fid,'%.6f]\n',data_paras_all(k,end));
end
fclose(o_fid);
